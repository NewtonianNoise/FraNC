...
class WavePacketFitMetric(EvaluationMetricPlottable):
    ...
    def result_full(self) -> tuple[float,]:
        if self.dataset.signal_evaluation is None:
            raise RuntimeError(
                "This metric is not supported on datasets without a signal component."
            )
            ...
            for offset, position, width, amplitude, frequency, phase in true_parameters:
                ...
                def model(_xdata, offset, width, amplitude, frequency, phase):
                    ...
                    return generate_wave_packet(
                        ...
                        frequency,
                        phase,
                        fixed_width=fixed_width,  # pylint: disable=cell-var-from-loop
                    )
        ...
        parameters_true_npy = np.array(parameters_true).T
        parameters_fit_npy = np.array(parameters_fit).T
        ...
        for idx, parameter in enumerate(
            ...
            plt.figure()
            plt.scatter(
                parameters_true_npy[idx],
                parameters_fit_npy[idx] - parameters_true_npy[idx],
            )
